...
int main(int argc, char *argv[])
{
    ...
    int i;
    //loop to check for bad character in cipher file
    ...
    strcat(plainBuffer, "#"); //add # to end of buffer to indicate end of file
    numReadPlain++;
    ...
    //loop to find bad char in key buffer
    ...
    strcat(keyBuffer, "#"); //add # to end of buffer to indicate end of file
    numReadKey++;

    //if cipher text is longer than key text return an error
    if (numReadPlain > numReadKey)
    {
        error("CLIENT: ERROR key too short");
        return -1;
    }
    //if unable to send cipher text return an error
    if ((charsWritten = send(socketFD, plainBuffer, numReadPlain, 0)) < 0)
    {
        error("CLIENT: ERROR writing to socket");
        return -1;
    }
    //if unable to send keytext return error
